...
SELECT EMPNO NO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, D.DEPTNO, DNAME, LOC  -- (3)
    ...
    ORDER BY NO;                                  -- (4)
    -- EX1. 모든 사원의 사번 이름 잡 상사사번 부서번호 부서명 근무지
    SELECT EMPNO,ENAME,JOB,MGR,D.DEPNO,DNAME,LOC
        FROM EMP E ,DEPT D
        WHERE E.DEPTNO=D.DEPTNO;
    --EX2 급여가(SAL) 2000이상인 직원의 이름,JOB,급여,부서명,근무지
    SELECT ENAME,JOB,SAL,DMANE,LOC
...
-- ★ 1. NON-EQUI JOIN(공통필드 값이 일치되는 조건만 JOIN)
...
----------------------- 
    ...
    SELECT W.EMPNO, W.ENAME, W.MGR, M.ENAME MANAGER
        FROM EMP W , EMP M
        WHERE W.MGR=M.EMPNO;
    --EX2. 'SMITH'의 상사는 JONES 이다. 포맷으로 출력
    SELECT W.ENAME || ' 의 상사는' || M.ENAME || '이다' MESSAGE 
        FROM EMP W,EMP M
        WHERE W.MGR=M.EMPNO;
    --탄탄 1: 매니저가 KING인 사원들의 이름과 직급을 출력하세요. 
