...
-- ★ 제약조건: INSERT, UPDATE 할때 제약조건에 맞는 데이터만 INSERT, UPDATE됨.
--(1) PRIMARY KEY :테이블의 각 행을 유일한 값으로 식벽하기 위한 필드 
--(2) FOREIGN KEY: 테이블의 열이 다른 테이블의 열을 참조
--(3) NOT NULL : NULL을 포함하지 않는 필드
--(4) UNIQUE: 모든 행의 값이 유일한 값, NULL은 허용(NULL은 여러개 입력가능)
--(5) CHECK(조건): 해당 조건이 만족(NULL 값 허용)
--DEFAULT 기본값: 해당열의 데이터를 입력하지 않으면 원래는 NULL이 들어갈 것을 기본값이 들어가도록 

-- DEPT & EMP1(ERD 작성 ->DDL EXERD를 이용하여 ERD 작성=테이블설계 ->
DROP TABLE DEPT1;
CREATE TABLE DEPT1(
    DEPTNO NUMBER(2) PRIMARY KEY,
...
CREATE TABLE EMP1(
 ...
 INSERT INTO EMP1 (EMPNO, ENAME, JOB, SAL, DEPTNO)
    VALUES (1111, 'HONG', 'MANAGER', 1000, 10);   
